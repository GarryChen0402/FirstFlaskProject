...
from flask import Flask, url_for, request, render_template, make_response, session, redirect, jsonify
...
@app.route('/book/<id>')
@app.route('/book/<int:id>') # <int:id> 是一个整数类型的转换器，用于将路由接收的参数id转换为int型， 此外， 还支持 string(default), int, float, path, uuid等类型
def show_book(id):
    print(type(id))
